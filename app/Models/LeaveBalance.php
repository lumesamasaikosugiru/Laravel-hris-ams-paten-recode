...
class LeaveBalance extends Model
{
    ...
    {
        // Hanya pegawai AKTIF — probation tidak dapat saldo
        $employees = Employee::where('status', 'active')->get();
        $leaveTypes = LeaveType::active()->get();

        foreach ($employees as $emp) {
            foreach ($leaveTypes as $lt) {
                if ($lt->gender !== 'all' && $lt->gender !== $emp->gender)
                    continue;
                if (
                    $lt->cycle === 'once' && self::where('employee_id', $emp->id)
                        ->where('leave_type_id', $lt->id)->exists()
                )
                    continue;
