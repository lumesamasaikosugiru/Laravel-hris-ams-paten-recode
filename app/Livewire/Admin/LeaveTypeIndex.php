...
class LeaveTypeIndex extends Component
{
    ...
    protected function rules(): array { return ['name'=>'required|max:255','quota'=>'required|integer|min:1|max:365','gender'=>'required|in:all,male,female','cycle'=>'required|in:annual,once','requires_document'=>'boolean','description'=>'nullable|max:500','is_active'=>'boolean']; }
    protected $messages = ['name.required'=>'Nama jenis cuti wajib diisi.','quota.required'=>'Kuota hari wajib diisi.'];

    public function openCreate(): void { $this->reset(['editingId','name','description']); $this->quota = 12; $this->gender = 'all'; $this->cycle = 'annual'; $this->requires_document = false; $this->is_active = true; $this->resetValidation(); $this->showModal = true; }

    public function openEdit(int $id): void {
        $lt = LeaveType::findOrFail($id);
        $this->editingId = $lt->id; $this->name = $lt->name; $this->quota = $lt->quota; $this->gender = $lt->gender;
        $this->cycle = $lt->cycle; $this->requires_document = $lt->requires_document; $this->description = $lt->description ?? ''; $this->is_active = $lt->is_active;
    ...
    public function delete(): void { LeaveType::findOrFail($this->deletingId)->delete(); session()->flash('success','Jenis cuti dihapus.'); $this->showDeleteModal = false; }
    public function toggleStatus(int $id): void { $lt = LeaveType::findOrFail($id); $lt->update(['is_active'=>!$lt->is_active]); session()->flash('success','Status diubah.'); }
