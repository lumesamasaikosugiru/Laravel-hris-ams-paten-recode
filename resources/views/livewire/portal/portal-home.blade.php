<div class="p-4 space-y-4">
    ...
    <div class="portal-card p-5" style="background: linear-gradient(135deg, #1a1040, #7c3aed);">
        <p class="text-white/70 text-sm">Selamat {{ now()->hour < 12 ? 'Pagi' : (now()->hour < 17 ? 'Siang' : 'Malam') }},</p>
        ...
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-semibold text-gray-700">
                    @if($todayAttendance->status === 'present') ✅ Hadir
                    @elseif($todayAttendance->status === 'late') ⏰ Terlambat
                    @elseif($todayAttendance->status === 'leave') 🌴 Cuti/Izin
                    @else ❌ Tidak Hadir
                    @endif
                </p>
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="portal-card p-4 text-center">
            ...
            <p class="text-xs text-gray-500 mt-1">Sisa Cuti Tahunan</p>
