...
$attPct = $totalActive > 0
    ? round(($attToday['present'] + $attToday['late']) / $totalActive * 100)
    : 0;

// ── Cuti ─────────────────────────────────────────────────────
$leavePending  = LeaveRequest::where('status','pending')->count();
$leaveApproved = LeaveRequest::where('status','approved')
    ->whereYear('start_date',$thisYear)->count();

// ── Rekrutmen ────────────────────────────────────────────────
$activeJobs      = JobVacancy::where('status','open')->count();
$newApplicants   = Applicant::where('status','submitted')
    ...
    <div class="card p-5">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">{{ $attPct }}% dari total · {{ $attToday['absent'] }} tidak hadir</p>
            </div>
    ...
    <div class="card p-5">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold {{ $overdueProb > 0 ? 'text-red-600' : 'text-gray-800' }} mt-1">{{ $totalProbation }}</p>
                <p class="text-xs {{ $overdueProb > 0 ? 'text-red-500 font-medium' : 'text-gray-400' }} mt-1">
                    @if($overdueProb > 0) {{ $overdueProb }} perlu evaluasi segera!
                    @elseif($endingSoon > 0) {{ $endingSoon }} berakhir dalam 7 hari
                    @else Semua on-track
                    @endif
                </p>
            </div>
    ...
    <div class="card p-5 lg:col-span-2">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('admin.attendance.index') }}"
               class="text-xs text-violet-600 hover:underline">Lihat semua →</a>
        </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
    ...
    <div class="card p-5">
        ...
        <div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-0">
            <div class="w-7 h-7 rounded-full bg-red-100 flex items-center justify-center text-red-600 text-xs font-bold shrink-0">
    ...
    <div class="card p-5">
        ...
        <div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-0">
            <div class="w-7 h-7 rounded-full bg-amber-100 flex items-center justify-center text-amber-600 text-xs font-bold shrink-0">
    ...
    <div class="card p-5">
        ...
        <div class="space-y-2">
            ...
            <a href="{{ $url }}" class="flex items-center gap-3 p-2.5 rounded-lg {{ $colors }} hover:opacity-80 transition">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(ctx, {
        ...
        data: {
            labels,
            datasets: [
                { label: 'Hadir',       data: present, backgroundColor: '#4ade80', borderRadius: 4 },
                { label: 'Terlambat',   data: late,    backgroundColor: '#fbbf24', borderRadius: 4 },
                { label: 'Tidak Hadir', data: absent,  backgroundColor: '#f87171', borderRadius: 4 },
            ]
        },
        options: {
            responsive: true,
