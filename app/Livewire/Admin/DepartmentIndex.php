...
class DepartmentIndex extends Component
{
    ...
    public function openCreate(): void { $this->reset(['editingId','school_id','code','name','description']); $this->is_active = true; $this->resetValidation(); $this->showModal = true; }

    public function openEdit(int $id): void {
        $d = Department::findOrFail($id);
        $this->editingId = $d->id; $this->school_id = $d->school_id;
        $this->code = $d->code; $this->name = $d->name; $this->description = $d->description ?? ''; $this->is_active = $d->is_active;
    ...
    public function save(): void {
        ...
        if ($exists) { $this->addError('code','Kode sudah dipakai di sekolah ini.'); return; }
    ...
    public function delete(): void { Department::findOrFail($this->deletingId)->delete(); session()->flash('success','Departemen dihapus.'); $this->showDeleteModal = false; }
    public function toggleStatus(int $id): void { $d = Department::findOrFail($id); $d->update(['is_active'=>!$d->is_active]); session()->flash('success','Status diubah.'); }
